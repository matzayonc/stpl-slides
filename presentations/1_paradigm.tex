...
%       Sequential reveal: Read (overlays 1-4), then Write (overlays 5-8).
  ...
  \begin{columns}
    \column{0.48\textwidth}
      \begin{block}{Read}
        ...
        \vspace{0.1cm}
        \begin{itemize}
          \item<1-> Gratis, bez podpisu
          \item<2-> Dowolny węzeł RPC
          \item<3-> Odpowiedź natychmiast
          \item<4-> Np. podgląd salda konta
        \end{itemize}
      \end{block}
    ...
    \column{0.48\textwidth}
      \begin{alertblock}{Write}
        ...
        \begin{tikzpicture}[
          ...
          % Layout reserved from <1->; nodes hidden until Write stages.
          \node[b, visible on=<5->] (c) {Klient};
          \node[b, right=0.2cm of c, visible on=<5->] (r) {RPC};
          \node[b, right=0.2cm of r, visible on=<6->] (k) {Klaster};
          \node[bk, right=0.2cm of k, visible on=<7->] (ok) {ack};
          % Stage 5: Klient → RPC
        ...
        \vspace{0.1cm}
        \begin{itemize}
          \item<5-> \textbf{Transakcja} podpisana kluczem
          \item<6-> Opłata sieciowa za włączenie
          \item<7-> Czeka na potwierdzenie klastra
          \item<8-> Np. wysłanie tokena
        \end{itemize}
      \end{alertblock}
  \end{columns}
\end{frame}
