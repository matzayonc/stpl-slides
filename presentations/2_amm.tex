...
\begin{frame}{Order book vs AMM}
  % Both columns open with a block that self-reveals via \pause (see preamble).
  % On a block-first frame those leading pauses push the first bullet to overlay
  % 4, leaving three blank opening slides. Pull the pause counter back so the
  % first bullet lands on overlay 1 instead.
...
\begin{frame}{Ryzyka w AMM}
  % The frame's only content is this block, which self-reveals via \pause (see
  % preamble), leaving a blank opening slide. Pull the pause counter back so the
  % block and its first bullet land on overlay 1.
